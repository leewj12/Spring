...
public class BoardController {
    ...
    public String boardWrite(BoardDTO board, Model model,
        ...
        if (board.getMode().equals("write")) {
            log.info("방금 쓴 글의 글 번호 1: {}", board.getNum()); // 0
//            for(int i =0;i<30; i++)
            n = boardService.insertBoard(board);// 글쓰기 처리

            log.info("방금 쓴 글의 글 번호 2: {}", board.getNum()); //시퀀스 증가값
    ...
    public String boardList2(Model model, PagingDTO paging, HttpServletRequest req) {
        ...
        //return "board/boardList";//c:forEach로 페이지 처리
        return "board/boardList2";//pageNavi 문자열을 출력
    }
...
//    }
    ...
    @GetMapping("/board/view")
    public String boardView(Model model, @RequestParam(defaultValue = "0") int num) {//설정에서 -param주면 자동 설정?
    ...
    public String boardPasswdConfirm(
        ...
        ServletContext ctx = req.getServletContext();//application객체
