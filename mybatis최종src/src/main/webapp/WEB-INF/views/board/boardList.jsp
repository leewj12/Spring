...
<html>
    ...
    <div class="wrap">
        ...
        <c:if test="${paging.findKeyword!=''}">
            <h3 style="text-align:center">검색어 :
            <c:out value="${paging.findKeyword}"/> </h3>
        </c:if>
        <br>
        ...
        <c:set var="qstr"
        value="&findType=${paging.findType}&findKeyword=${paging.findKeyword}"/>
        <%-- <h1>${qstr}</h1> --%>
    ...
    <script>
        const find_check=()=>{
            ...
            if(!findF.findKeyword.value.trim()){
                alert('검색어를 입력하세요');
